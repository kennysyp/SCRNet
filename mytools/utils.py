...
class Logger(object):
    def __init__(self, save_dir, file_name=''):
        ...
        if file_name == '':
            self.log = open(save_dir + "train.log", "a")
        else:
            self.log = open(save_dir + 'test_' + file_name + ".log", "a")
...
def dice_val_VOI(y_pred, y_true, VOI_lbls):
    ...
    for i in VOI_lbls:
        ...
        # Written specifically for Abdomen MR-CT
        if ((true == i).sum() == 0) or ((pred == i).sum() == 0):
            # DSCs[idx] = np.nan
            '''Follow line 43 of https://github.com/Runshi-Zhang/UTSRMorph/blob/main/submit_utsrmorph_abdomen.py'''
            DSCs[idx] = 1
        else:
            intersection = pred_i * true_i
            intersection = np.sum(intersection)
            ...
            DSCs[idx] =dsc
        idx += 1
    return np.nanmean(DSCs)  # Written specifically for Abdomen MR-CT

# def metric_val_VOI(y_pred, y_true, VOI_lbls):
#     pred = y_pred.detach().cpu().numpy()[0, 0, ...]
#     true = y_true.detach().cpu().numpy()[0, 0, ...]
#     # Lists_dsc = np.zeros((len(VOI_lbls), 1))
#     Lists_hd = np.zeros((len(VOI_lbls), 1))
#     Lists_asd = np.zeros((len(VOI_lbls), 1))
#     DSCs = np.zeros((len(VOI_lbls), 1))  # dice_val_VOI
#     idx = 0
#     for i in VOI_lbls:
#         pred_i = pred == i
#         true_i = true == i
...
#         Lists_asd[idx] = asd
#
#         intersection = pred_i * true_i  # dice_val_VOI
#         intersection = np.sum(intersection)  # dice_val_VOI
#         union = np.sum(pred_i) + np.sum(true_i)  # dice_val_VOI
#         dsc = (2. * intersection) / (union + 1e-5)  # dice_val_VOI
#         DSCs[idx] = dsc  # dice_val_VOI
#
#         idx += 1
#     # return np.mean(Lists_dsc), np.mean(Lists_hd), np.mean(Lists_asd)
#     return np.mean(DSCs), np.mean(Lists_hd), np.mean(Lists_asd)
...
def metric_val_VOI(y_pred, y_true, VOI_lbls):
    ...
    for i in VOI_lbls:
        ...
        idx += 1
    return np.nanmean(DSCs), np.nanmean(Lists_hd), np.nanmean(Lists_asd)
