...
def normalized_cross_correlation(img1, img2):
    ...
    # 计算ncc
    result = torch.mean((img1 - mean_img1) * (img2 - mean_img2) / (std_img1 * std_img2))

    return result



def flow_difference_loss(initial_flow, optimized_flow, alpha=0.7, beta=0.3):
    """
    方向一致性损失 + 幅度正则化
    """
    # 1. 方向一致性损失 (核心)
    dot_product = torch.sum(initial_flow * optimized_flow, dim=1)  # 沿第1维（通道维，即3个方向）求和。从 [b, 3, d, h, w]变为[b, d, h, w]
    norm_initial = torch.norm(initial_flow, p=2, dim=1)  # 沿第1维计算L2范数（即向量的模长）
    norm_optimized = torch.norm(optimized_flow, p=2, dim=1)  # 沿第1维计算L2范数（即向量的模长）
    cosine_sim = dot_product / norm_initial * norm_optimized  # 计算余弦相似度：cosine_sim = dot_product / (||u|| * ||v||)。
    # 余弦相似度的范围在[-1,1]之间，1表示方向完全相同，-1表示方向完全相反。那么1-mean(cosine_sim)的话，表示在方向相同时为0，方向相反时为2。
    direction_loss = 1 - torch.mean(cosine_sim)  # 最小化这个损失就是最大化方向一致性。

    # 2. 相对幅度变化正则化
    magnitude_ratio = norm_optimized / norm_initial
    magnitude_loss = torch.mean((magnitude_ratio - 1) ** 2)  # 惩罚过度变化

    # 3. 组合损失
    return alpha * direction_loss + beta * magnitude_loss
...
class DeformationOptimizer(nn.Module):
    def __init__(self, img_size, initial_flow, mode='bilinear'):
        ...
        self.spatial_trans = SpatialTransformer(self.img_size, self.mode)
